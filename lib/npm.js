...
NPMLocation.prototype = {
  ...
  processPackageConfig: function(packageConfig, packageName) {
    if (packageConfig.jspmNodeConversion === false || packageConfig.jspmPackage === true)
    ...
    // by default we install optionalDependencies, although this can be overridden by a standard dependency override
    ...
    if (packageConfig.directories && packageConfig.jspm && packageConfig.jspm.directories && packageConfig.jspm.directories.lib && !packageConfig.directories.dist) {
      packageConfig.directories.dist = packageConfig.directories.lib;
      packageConfig.jspm.directories.dist = pacakgeConfig.jspm.directories.lib
  ...
  processPackage: function(packageConfig, packageName, packageDir) {
    // NB deprecate this property with the package.json systemjs: true / systemjs: { ... } config
    if (packageConfig.jspmNodeConversion === false || packageConfig.jspmPackage === true)
      return packageConfig;
