...
var nodeCoreModules = {
  'assert': 'npm:assert@^1.3.0', // (fork required)
  ...
  'sys': 'npm:util@^0.10.3',
  'timers': 'npm:timers-browserify@^1.4.2',
  'tls': 'nodelibs/tls',
  'tty': 'npm:tty-browserify@^0.0.0', // https://github.com/substack/tty-browserify/pull/1
  'url': 'npm:url@^0.11.0', // (fork required)
  'util': 'npm:util@^0.10.3', // (fork required)
  ...
  'nodelibs': 'github:jspm/nodelibs@^0.1.0' // psuedo-core
};
  ...
  .then(function(fileTree) {
    ...
    if (map) {
      ...
      if (!map['./' + systemConfig.main]) {
        ...
        if (browserMain) {
          ...
          if (browserMain != systemConfig.main)
    ...
    return Promise.all(Object.keys(fileTree).filter(function(file) {
      ...
      // otherwise read format comprehensively from existing metadata
        ...
        var fileFormat = readMeta(fileName, systemConfig.meta).format || systemConfig.format;
      ...
      .then(function(detectedFormat) {
        ...
        if (meta && detectedFormat && detectedFormat != systemConfig.format)
        ...
        .then(function(source) {
          var parsed = parseCJS(source.toString());

          // add buffer global for CJS files that need it
          if (meta && parsed.usesBuffer) {
            curMeta.globals = curMeta.globals || {};
          ...
          if (map && fileName.substr(fileName.length - 9, 9) == '/index.js' && !fileTree[fileName.substr(0, fileName.length - 9) + '.js'])
          ...
          parsed.requires.forEach(function(req) {
            ...
            else if (meta && nodeResolved.substr(nodeResolved.length - 3, 3) != '.js')
    ...
    .then(function() {
      ...
      if (meta && hasProperties(meta)) {
        ...
        // ensure all skip extensions either have meta or true
        skipExtensions.forEach(function(m) {
          if (!meta[m] || !hasProperties(meta[m]))
            meta[m] = true;
        });
