  ...
  return new Promise(function(resolve, reject) {
    ...
    readdirp({
      ...
      directoryFilter: function(dir) {
        ...
        if (dir.path.match(/^(\.git|node_modules|jspm_packages)($|\/)/)) {
          return false;
        }
        // ignores
        if (packageConfig.ignore instanceof Array) {
          if (packageConfig.ignore.some(function(ignore) {
            return dir.path.substr(0, ignore.length) == ignore && (dir.path.length == ignore.length || dir.path[ignore.length] == '/');
          }))
            return false;
        }
        // files
        if (packageConfig.files instanceof Array) {
          return packageConfig.files.some(function(file) {
            ...
            return dir.path.substr(0, file.length) == file && (dir.path.length == file.length || dir.path[file.length] == '/');
          });
        }
        return true;
      }
      ...
      .then(function(detectedFormat) {
        ...
        .then(function(source) {
          ...
          parsed.requires.forEach(function(req) {
            ...
            else if (meta && nodeResolved.substr(nodeResolved.length - 3, 3) != '.js' && nodeResolved != '.')
