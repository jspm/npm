    ...
    .then(function() {
      ...
      function inMapTargets(dep) {
        return Object.keys(systemConfig.map).some(function(map) {
          ...
          return target.substr(0, dep.length) == dep && (target[dep.length] == '/' || target.length == dep.length);
        });
      }

      systemConfig.peerDependencies = systemConfig.peerDependencies || {};
      coreDeps.sort().forEach(function(dep) {
        if (systemConfig.map && systemConfig.map[dep] && typeof systemConfig.map[dep] == 'string' && !inMapTargets(dep) || 
            systemConfig.peerDependencies && systemConfig.peerDependencies[dep] || systemConfig.dependencies && systemConfig.dependencies[dep])
          return;
