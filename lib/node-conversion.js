...
var nodeCoreModules = {
  'assert': 'npm:assert@^1.3.0', // REFUSED https://github.com/defunctzombie/commonjs-assert/pull/11
  ...
  'crypto': 'npm:crypto-browserify@^3.11.0', // REFUSED https://github.com/crypto-browserify/crypto-browserify/pull/144
  'dgram': 'nodelibs/dgram',
  'dns': 'nodelibs/dns',
  'domain': 'npm:domain-browser@^1.1.4', // https://github.com/bevry/domain-browser/pull/7
  'events': 'npm:events@^1.1.0', // REFUSED https://github.com/Gozala/events/pull/25
  'fs': 'nodelibs/fs',
  ...
  'url': 'npm:url@^0.11.0', // REFUSED https://github.com/defunctzombie/node-url/pull/23
  'util': 'npm:util@^0.10.3', // REFUSED https://github.com/defunctzombie/node-util/pull/8
...
exports.convertPackage = function(pjson, packageName, packageDir, ui) {
  ...
  .then(function(fileTree) {
    ...
    return Promise.all(Object.keys(fileTree).filter(function(file) {
      ...
      return parseModule(path.resolve(packageDir, fileName), existingFormat)
      .then(function(parsed) {
        ...
        requires.forEach(function(req) {
          ...
          if (nodeResolved.substr(nodeResolved.length - 5, 5) == '.json') {
            // the *.json meta will handle this case for us otherwise
            if (map && req.substr(req.length - 5, 5) != '.json' && (meta || packageConfig.meta && packageConfig.meta['*.json']))
    ...
    .then(function() {
      ...
      // if any package properties reference "nodelibs", include "nodelibs" as a dependency
      // every npm package depends on github:jspm/nodelibs as a peer dependency
      // (lazy detection method!)
      if (JSON.stringify(packageConfig).indexOf('nodelibs') != -1) {
