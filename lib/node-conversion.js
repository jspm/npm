    ...
    if (typeof pjson.browser == 'object')
      for (var b in pjson.browser) {
        ...
        if (mapping === false) {
          ...
          mapResolve = nodeResolve(mapping, '', fileTree);
          if (mapResolve)
            mapping = './' + mapResolve;
        }
    ...
    return Promise.all(Object.keys(fileTree).filter(function(file) {
      ...
      .then(function(parsed) {
        ...
        requires.forEach(function(req) {
          ...
          if (nodeResolved.substr(nodeResolved.length - 9) == '/index.js' && req.substr(req.length - 9) != '/index.js' && req.substr(req.length - 6) != '/index')
